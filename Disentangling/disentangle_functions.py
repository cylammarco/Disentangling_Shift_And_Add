...
def Normalise(spec, points=[]):
    ContIndices = np.array(
        [
            np.argmin(np.abs(spec[:, 0] - points[i]))
    ...
    Contfluxes = np.array(
        [
            np.average(
                spec[:, 1][
                    max(ContIndices[i] - 7, 0) : min(
                        ContIndices[i] + 7, len(spec[:, 0])
                    )
                ]
            )
            for i in np.arange(len(ContIndices))
        ]
    )
...
def Cosclean(
    ...
    for itr in range(10):
        ...
        if not len(flagged_args):
            print(
                "There are no cosmics detected with the given threshhold and size"
            )
            return Spec
        blimit = 0
        N = len(waves)
        for i in flagged_args:
            ...
            argr = ipeak + argr[0]
            if (
                abs(fluxes[argb] - fpeak) < sigmab
                or abs(fluxes[argr] - fpeak) < sigmar
            ):
                continue
            Spec[argb : argr + 1, 1] = np.interp(
                waves[argb : argr + 1],
...
def v1andv2(nu, Orbital_Params):
    (
        ...
        K3,
        K4,
    ) = list(Orbital_Params.values())
    Omegapi = omega / 180.0 * np.pi
    OmegapiOut = omegaOut / 180.0 * np.pi
    Omegapi_2 = omega_2 / 180.0 * np.pi
...
# nu = DoF
def Chi2con(
    redchi2, nu, K1s, K2s, Rangestr, P1=0.68, comp="secondary", ParbSize=3
):
    if comp == "secondary":
        Kscomp = np.copy(K2s)
    else:
    ...
    if a < 0:
        print(
            "Could not fit sensible parabola (a < 0); try changing the fitting range by increasing/decreasing ParbSize argument"
        )
        plt.scatter(Kscomp, redchi2)
        plt.show()
        return 0.0, Kscomp[indmin], 0.0
    ParbMin = c - b**2 / 4.0 / a
    ...
    ys1 = sc.gammainc(nu / 2.0, xarr / 2) - P1
    ...
    K2min = -b / 2.0 / a
    K2err = K2min - (-b - np.sqrt(b**2 - 4 * a * (c - chi2P1))) / 2.0 / a
    plt.plot(
        [Kscomp[0], Kscomp[-1]],
        [chi2P1, chi2P1],
        color="red",
    ...
    if comp == "secondary":
        ...
        np.savetxt(
            "Output/" + Rangestr + "_" + "grid_dis_K2.txt",
            np.c_[Kscomp, redchi2],
            header="#1sigma = " + str(chi2P1 * ParbMin),
        )
        plt.savefig(
            "Output/" + Rangestr + "_Grid_disentangling_K2.pdf",
            bbox_inches="tight",
        )
    else:
        plt.xlabel(r"$K_1$ [km/s]")
        np.savetxt(
            "Output/" + Rangestr + "_" + "grid_dis_K1.txt",
            np.c_[Kscomp, redchi2],
            header="#1sigma = " + str(chi2P1 * ParbMin),
        )
        plt.savefig(
            "Output/" + Rangestr + "_Grid_disentangling_K1.pdf",
            bbox_inches="tight",
        )
    plt.show()
...
def Reduce_Waves(waves, nusdata, Orbital_Params, K1max, K2max):
    ...
    Inds = np.where(np.diff(waves) > 1.0)
    WaveCalcCond = waves < 0
    if len(Inds) == 0:
        LamMin = waves[0] * (1.0 + max(max(vrA), max(vrB)) / clight)
        LamMax = waves[-1] * (1.0 + min(min(vrA), min(vrB)) / clight)
        ...
        for j in np.arange(len(Inds) - 1):
            LamMin = waves[Inds[j] + 1] * (
                1.0 + max(max(vrA), max(vrB)) / clight
            )
            LamMax = waves[Inds[j + 1]] * (
                1.0 + min(min(vrA), min(vrB)) / clight
            )
...
def Prepare_Plot_Extremes(
    ...
    axes[Panel].plot(
        ...
        color="red",
        linestyle="dotted",
        linewidth=linewidExt,
    )
    axes[Panel].plot(
        waves, Bshift, label="Sec. Dis.", color="green", linewidth=linewidExt
    )
    if NebLines:
    ...
    if Panel == 1:
        ...
        try:
            ...
            NameFile = (
                ...
                + Rangestr
                + "_Extremes_"
                + str(np.round(Orbital_Params["K1"]))
                + "_"
                + str(np.round(K2s[kcount]))
                + ".pdf"
            )
            plt.savefig(NameFile, bbox_inches="tight")
        plt.show()
...
def CalcDiffs(
    ...
    if "DoFs" not in globals():
        DoFs = 1
    WaveCalcCond = Reduce_Waves(
        waves, nusdata, Orbital_Params, K1s[-1], K2s[-1]
    )
    Sum = 0
    ...
    for ind in np.arange(len(ObsSpecs)):
        ...
        Ashift = interp1d(
            waves * Facshift1, A, bounds_error=False, fill_value=0.0
        )(waves[WaveCalcCond])
        Bshift = interp1d(
            waves * Facshift2, B, bounds_error=False, fill_value=0.0
        )(waves[WaveCalcCond])
        ObsSpec = interp1d(
            ObsSpecs[ind][:, 0],
            ObsSpecs[ind][:, 1] - 1,
        ...
        if NebLines:
            ...
            Nebshift = (
                NebFac
                * ScalingNeb[ind]
                * interp1d(waves, NebSpec, bounds_error=False, fill_value=0.0)(
                    waves[WaveCalcCond]
                )
            )
        ...
        sigma = (
            np.std(ObsSpec[:S2NpixelRange]) + np.std(ObsSpec[-S2NpixelRange:])
        ) / 2.0
        if Resid:
            ...
            if kcount == kcount_extremeplot:
                if ind == min(RVExtminInd, RVExtmaxInd):
                    fig, axes = plt.subplots(
                        nrows=2, ncols=1, figsize=ExtremesFigSize
                    )
                    Prepare_Plot_Extremes(
                        fig,
                        axes,
                        waves[WaveCalcCond],
                        Ashift,
    ...
    if not ShowItr:
        print(
            "chi2:", Sum / (len(waves[WaveCalcCond]) * len(ObsSpecs) - CompNum)
        )
    if Resid:
        return Residuals
    if Reduce:
...
def disentangle(
    ...
    if PLOTCONV:
        StrictNegA, StrictNegB, StrictNegC, StrictNegD = (
            False,
            False,
            False,
            False,
    ...
    WavesBA = np.array(
        [waves * Facshift2[i] / Facshift1[i] for i in np.arange(len(vrads1))]
    )
    # Frame of Refernce star 2:
    WavesAB = np.array(
        [waves * Facshift1[i] / Facshift2[i] for i in np.arange(len(vrads1))]
    )
    if NebLines:
        FacshiftNeb = np.ones(len(vrads1))
        SsNeb = [
            interp1d(
                ObsSpecs[i][:, 0] / FacshiftNeb[i],
                ObsSpecs[i][:, 1] - 1.0,
                bounds_error=False,
                fill_value=0.0,
                kind=InterKind,
        ...
        WavesNebA = np.array(
            [
                waves * FacshiftNeb[i] / Facshift1[i]
                for i in np.arange(len(vrads1))
            ]
        )
        WavesNebB = np.array(
            [
                waves * FacshiftNeb[i] / Facshift2[i]
                for i in np.arange(len(vrads1))
            ]
        )
        WavesANeb = np.array(
            [
                waves * Facshift1[i] / FacshiftNeb[i]
                for i in np.arange(len(vrads1))
            ]
        )
        WavesBNeb = np.array(
            [
                waves * Facshift2[i] / FacshiftNeb[i]
                for i in np.arange(len(vrads1))
            ]
        )
    itr = 0
    while itr < itrnumlim:
        ...
        if NebLines:
                ...
                np.array(
                    [
                        ...
                        * (
                            Ss1[i]
                            - BAshifts[i](waves)
                            - NebFac * ScalingNeb[i] * NebAshifts[i](waves)
                        )
            ...
            SpecMean = np.sum(
                np.array(
                    [
                        weights[i] * (Ss1[i] - BAshifts[i](waves))
                        for i in np.arange(len(Ss1))
                    ]
                ),
                axis=0,
        ...
        Anew = interp1d(
            waves, SpecMean, bounds_error=False, fill_value=0.0, kind=InterKind
        )(waves)
        if StrictNegA:
        ...
        if NebLines:
                ...
                np.array(
                    [
                        ...
                        * (
                            Ss2[i]
                            - ABshifts[i](waves)
                            - NebFac * ScalingNeb[i] * NebBshifts[i](waves)
                        )
                        for i in np.arange(len(Ss1))
                    ]
                ),
            ...
            SpecMean = np.sum(
                np.array(
                    [
                        weights[i] * (Ss2[i] - ABshifts[i](waves))
                        for i in np.arange(len(Ss1))
                    ]
                ),
                axis=0,
        ...
        if NebLines:
            ...
            NebSpecnew[NebSpecnew < Poslimall[0]] = 0.0
        ...
        if ShowItr:
            if itr % 100 == 0:
                print(
                    ...
                    + str(itrnumlim)
                    + " iterations ("
                    + str(np.round(itr / itrnumlim * 100.0, 3))
                    + "%)"
                )
    ...
    return DisSpecVector + 1.0, CalcDiffs(
        DisSpecVector,
        vrads1,
        vrads2,
        waves,
...
def Grid_disentangling2D(
        ...
        for k1, K1 in enumerate(K1s):
            for k2, K2 in enumerate(K2s):
                ...
                if Ini == "A":
                    ...
                    print(
                        "No initial approximation given, assuming flat spectrum for secondary..."
                    )
                    Bini = interp1d(
                        waves,
                        np.ones(len(waves)),
    ...
    try:
        ...
        StepSize2 = 0
    np.savetxt(
        "Output/" + Rangestr + "_" + "grid_dis_K1K2.txt",
        np.array(Diffs),
